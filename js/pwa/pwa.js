(function() {
	...
	function G(a, b) {
			function d(f, c, h, m) {
					...
					m && (c = m.width / e, e = m.height / e, 128 < e && (c /= e / 128, e = 128), 48 <= c &&
							48 <= e && (l.drawImage(m, c / -2, e / -2, c, e), l.translate(0, e / 2 + 20)));
					...
					m = c.j || 24;
					l.translate(0, m);
					if (c.width < .8 * f) l.fillText(e, c.width / -2, 0);
					else
							for (e = e.split(/\s+/g), c = 1; c <= e.length; ++c) {
									var H = e.slice(0, c).join(" "),
											I = l.measureText(H).width;
									if (c === e.length || I > .6 * f) l.fillText(H, I / -2, 0), l.translate(0, 1.2 * m), e.splice(0,
											c), c = 0
							}
					return function() {
			...
			var p = (a.icons || []).map(R).sort(function(f,
							c) {
							return c.h - f.h
					}),
			...
			W && (k("application-name", a.short_name), k("msapplication-tooltip", a.description), k("msapplication-starturl", b(a.start_url || ".")), k("msapplication-navbutton-color", a.theme_color), (q = q[0]) && k("msapplication-TileImage", b(q.src)), k("msapplication-TileColor", a.background_color));
			k("theme-color", a.theme_color);
			if (A) {
					var y = a.background_color || "#f8f9fa",
							S = N(y);
					(q = X(a.related_applications)) && k("apple-itunes-app", "app-id=" + q);
					k("apple-mobile-web-app-capable",
							p);
					...
					por: "portrait",
					lan: "landscape"
			} [String(a.orientation || "").substr(0, 3)] || "", k("x5-orientation", q), k("screen-orientation", q), "fullscreen" === M ? (k("x5-fullscreen", "true"), k("full-screen", "yes")) : p && (k("x5-page-mode", "app"), k("browsermode", "application"))
	}
	...
	function K(a, b, d) {
			...
			var g = D(a);
			g.drawImage(a, 0, 0);
			if (d || 255 !== g.getImageData(0, 0, 1, 1).data[3]) return g.globalCompositeOperation = "destination-over", g.fillStyle = b, g.fillRect(0, 0, a.width, a.height), g.canvas.toDataURL()
	}
	...
	if ("onload" in XMLHttpRequest.prototype && !navigator.m) {
			var U = ["standalone", "fullscreen", "minimal-ui"],
					...
					A = navigator.vendor && -1 !== navigator.vendor.indexOf("Apple") && (-1 !== P.indexOf("Mobile/") || "standalone" in navigator) || !1,
